...
class Appointment(models.Model):
    ...
    def __str__(self):
        return f"نوبت {self.patient.full_name} با {self.doctor.full_name}"
...
class MedicalRecord(models.Model):
    ...
    def __str__(self):
        return f"پرونده پزشکی برای {self.نوبت.patient.full_name}"
...
class Prescription(models.Model):
    ...
    def __str__(self):
        return f"{self.appointment.patient.full_name }  نسخه برای "
...
class Billing(models.Model):
    ...
    status = models.CharField(max_length=120, choices=[('Paid', 'پرداخت شده'), ('Unpaid', 'پرداخت نشده')])
